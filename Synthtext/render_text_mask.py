...
def render_normal(font, text):
    ...
    # font parameters:
    line_spacing = font.get_sized_height() + 1  # 字体高度的平均值

    # initialize the surface to proper size:
    line_bounds = font.get_rect(lines[np.argmax(lengths)])  # (文字左上角的x，y，文字宽度，文字高度)
    fsize = (round(2.0 * line_bounds.width), round(1.25 * line_spacing * len(lines)))  # 画布大小（2倍文字宽度，1.25倍字体平均高度）
    surf = pygame.Surface(fsize, pygame.locals.SRCALPHA, 32)  # ((宽，高),显示格式，色深)
    ...
    for l in lines:
        ...
        for i, ch in enumerate(l): # render each character
...
def lly_perspective(img):  # w first
    """将文字居中"""
    img_h, img_w = img.shape[:2]
    loc = np.where(img > 127)
    miny, minx = np.min(loc[0]), np.min(loc[1])  # 定位文本区域
    maxy, maxx = np.max(loc[0]), np.max(loc[1])
    text_w = maxx - minx + 1
    text_h = maxy - miny + 1

    ori_canvas = np.zeros_like(img).astype(np.uint8)  # 背景
    a = img_h // 2 - text_h // 2  # 文字左上角的y值
    b = img_w // 2 - text_w // 2  # 文字左上角的x值

    ori_canvas[a:a + text_h, b:b + text_w] = img[miny:maxy + 1, minx:maxx + 1]
    final_x, final_y = b, a

    position = [final_x, final_y, final_x + text_w, final_y + text_h]  # 文字部分的左上角坐标 和 右下角坐标

    return ori_canvas, position
...
def perspective(img, rotate_angle, zoom, shear_angle, perspect, pad): # w first
    ...
    # print(f'tlx:{tlx},tly:{tly}')
    # temp_img_path = 'C:/Users/LLY/PycharmProjects/SRNet-datagen/temp/temp1.png'
    # cv2.imwrite(temp_img_path, canvas, [int(cv2.IMWRITE_PNG_COMPRESSION), 0])
    ...
    # temp = img[miny-6:maxy + 1, minx-6:maxx + 1]
    # temp_img_path = 'C:/Users/LLY/PycharmProjects/SRNet-datagen/temp/temp4.png'
    # cv2.imwrite(temp_img_path, temp, [int(cv2.IMWRITE_PNG_COMPRESSION), 0])

    ori_canvas = np.zeros_like(img).astype(np.uint8)  # 背景
    a = img_h//2-text_h//2  # 文字左上角的y值
    b = img_w//2-text_w//2  # 文字左上角的x值
    # print(a, b, text_h, text_w, minx, miny, maxx, maxy)
    # ori_canvas[img_h//2-text_h//2:img_h//2-text_h//2+text_h+6, img_w//2-text_w//2:img_w//2-text_w//2+text_w+6] = img[miny-6:maxy + 1, minx-6:maxx + 1]
    ori_canvas[a:a+text_h, b:b+text_w] = img[miny:maxy+1, minx:maxx+1]
    final_x, final_y = b, a
    # print(f'final_x:{final_x},final_y:{final_y},rx:{final_x+text_w},ry:{final_y+text_h}')
    # temp_img_path = 'C:/Users/LLY/PycharmProjects/SRNet-datagen/temp/temp3.png'
    # cv2.imwrite(temp_img_path, ori_canvas, [int(cv2.IMWRITE_PNG_COMPRESSION), 0])
    position = [final_x, final_y, final_x+text_w, final_y+text_h]  # 文字部分的左上角坐标 和 右下角坐标
...
if __name__ == '__main__':
    freetype.init()
    font = '/home/lily/SRNet-datagen/Synthtext/data/fonts/cn_ttf/simfang.ttf'
    font = freetype.Font(font)
    font.antialiased = True
    font.origin = True
    font_size = [25, 40]
    font.size = np.random.randint(font_size[0], font_size[1] + 1)  # 随机一个字体大小
    text = '早上好'

    render_normal(font, text)
